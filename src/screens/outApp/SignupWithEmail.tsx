...
import { BG, MAIN_COLOR } from '../../assets/color.js'
import { KANIT_BOLD, KANIT_THIN, KANIT_MEDIUM, KANIT_REGULAR, KANIT_EXTRALIGHT, KANIT_ITALIC, KANIT_BLACK_ITALIC } from '../../assets/fonts/font.js'
import { isValidEmail } from '../../utils/checkEmail.js'
import axios from 'axios'
import { compareCodeApi, sendEmailApi } from '../../utils/API/email'
import { AppContext } from '../../context/AppContext.tsx'
import AlertModal from '../../components/AlertModal.tsx'
const { height, width } = Dimensions.get('window')
const SignupWithEmail = ({ navigation }: any) => {
    ...
    async function handleSendCode() {
        if (isValidEmail(email)) {
            setIsLoading(true);
            try {
                const res = await axios.post(sendEmailApi, {
                    mainId: email
                });
    ...
    async function handleVertifiedCode() {
        ...
        try {
            ...
            if (res.status == 200) {
                ...
                setUser((pre: any) => ({ ...pre, mainId: email }))
                ...
                <View style={{ gap: 25 }}>
                    <View>
                        ...
                        <TextInput
                            ...
                            style={styles.txtIn} />
                        {
                            !isValidEmail(email) && email != "" && <Text style={styles.errorTxt}>Your email is invalid</Text>
                        }
                    </View>
                    <TouchableOpacity style={styles.btn}
                        onPress={() => { handleSendCode() }}>
                        <Text style={styles.btnTxt}>Send code to my email</Text>
                    </TouchableOpacity>
                    <View>
